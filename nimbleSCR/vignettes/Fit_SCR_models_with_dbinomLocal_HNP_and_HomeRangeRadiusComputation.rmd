...
# source(file.path(FunD, "nimbleSCR_HRfunctions/dbinomLocal_HNP.R"))
...
save(myNimbleOutput, niter, burnin, nchains, MCMCRuntime,  
     file.path(baseDir,"nimbleSCR/inst/extdata/dbinomLocal_HNP_samples.RData"))
```


```{r echo = F}
if(is.null(baseDir)) {
    load(system.file("extdata", "dbinomLocal_HNP_samples.RData", package = "nimbleSCR"))
} else {
    load(file.path(baseDir,"nimbleSCR/inst/extdata/dbinomLocal_HNP_samples.RData"))
}
```
...
HRAnim.mat <- HRA_nimble(param = posteriorSamples, detfun = 1, pr = pr, d = 6, 
                         xlim = c(0, dim(habitatMask)[2]), ylim = c(0, dim(habitatMask)[1]),
                         ng = 800, tol = 1E-5, niter = 2000)
