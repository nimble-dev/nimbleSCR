...
set.seed(2)
#flip detectors upsidedown 
data$detector.xy <- data$detector.xy
data$detector.xy[,2] <- data$upperCoords[2]-data$detector.xy[,2]
...
#flip initial values upsidedown 
inits$sxy[,2] <- data$upperCoords[2]- inits$sxy[,2]
...
Rmodel <- nimbleModel(code, constants, data, inits)
 Rmodel$calculate()
# 
 which(is.infinite(Rmodel$logProb_y),arr.ind = T)
  which(is.infinite(Rmodel$logProb_ones),arr.ind = T)

# plot(data$detector.xy[,2]~data$detector.xy[,1])
# i=5
# Rmodel$sxy[i,]
# points(Rmodel$sxy[i,2]~Rmodel$sxy[i,1],col="red",pch=16)
# data$y[i,]
# dets <- data$yDets[i,1:data$nbDetections[i]]
# points(data$detector.xy[dets,2]~data$detector.xy[dets,1],col="blue",pch=16)
# 
# data$detector.xy[dets,]
# Rmodel$sxy[i,]
# 
# 
# i
# 
# my.jags.input$detector.xy[dets,]
# inits.1$sxy[i,]
