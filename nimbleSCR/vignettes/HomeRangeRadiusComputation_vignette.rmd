---
title: "Home range size estimation with the nimbleSCR package"
author: "Soumen Dey"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
bibliography: references.bib
vignette: >
  %\VignetteIndexEntry{Computation of home range area with a variety of detection functions}
...
Here, we use a custom distribution 'dbinomLocal_HNP()' (available in the nimbleSCR package) corresponding to the half-normal plateau detection function [@dey2022detfun].

Note that we do not provide 'detNums' and 'detIndices' arguments in the 'dbinomLocal_HNP()' function as we want to use this model to simulate data (see '?dbinomLocal_HNP' for further details). 
...
modelCode <- nimbleCode({
  ...
  for(i in 1:M) {
    ...
    z[i] ~ dbern(psi)
    ## likelihood
    y[i, 1:lengthYCombined] ~ dbinomLocal_HNP(
...
The half-normal plateau (HNP) detection function is written as:
\begin{align}
p_{\text{HNP}}(d) = 
\begin{cases}
...
The function 'HRA_nimble()' uses numerical approximation (to find the root) and estimate of $95\%$ home range radius and area for a set of circular detection functions available in nimbleSCR, viz., half-normal(HN, detfun = 0), half-normal plateau (HNP, detfun = 1), exponential (EX, detfun = 2), asymmetric logistic (AL, detfun = 3), bimodal (BI, detfun = 4), and donut (DN, detfun = 5). 

The function 'HRA_nimble()' has two parts: a setup and and a run part. If we only want to calculate HR radius for a single set of parameter values, there is no need to compile. But for multiple sets of parameter values (e.g., an mcmc sample) one should compile the function before implementing the run part to reduce the computation time. 'HRA_nimble()' returns the estimates of home range radius and area for a given set of arguments with respect to a particular detection function using bisection algorithm:

* Setup part

  - param: A vector or matrix (parameters in columns) of values for different parameters corresponding to the specified detection function.
  - xlim: A vector of length 2 giving the range along x-axis
  - ylim: A vector of length 2 giving the range along y-axis
  - ng: A numeric variable denoting the number of breaks along an axis
  ...
  - detfun: A numeric variable denoting the type of detection function: 0 = Half-normal (HN), 1 = Half-normal plateau (HNP), 2 = Exponential (EX), 3 = Aysmmetric logistic (AL), 4 = Bimodal (BI), 5 = Donut (DN). 
  - pr: A numeric variable denoting the quantile probability to compute the radius 
  - d: A numeric variable giving an inital value of the radius
...
Next we obtain estimate $95\%$ home range radius and area for half-normal plateau detection function. Note that, we only have to provide the parameter values for 'sigma' and 'w' (i.e., 'p0' is not needed) to compute home range radius and area.
...
HRAnim <- HRA_nimble( param = params, 
                      xlim = c(0, dim(habitatMask)[2]),
                      ylim = c(0, dim(habitatMask)[1]),
...
HRAnim.mat <- HRA_nimble(param = posteriorSamples, 
...
```
The numerical estimate of home range radius and area can be improved further by tuning the function arguments: the number of bins ('ng'), the tolerance ('tol') and the uppler limit of the number of iterations for numerical approximation using bi-section algorithm ('niter'). However, tuning of these parameters will also affect the computation time of HRA_nimble(). 

## 2. Calculating home range radius and area with other detection functions

We can also fit SCR models with half-normal or exponential detection function using custom distributions such as 'dbinomLocal_normal()' or 'dbinomLocal_exp()', respectively. An example of SCR model fitting using half-normal detection and other efficient nimbleSCR features is given [here](https://cran.r-project.org/web/packages/nimbleSCR/vignettes/Simulate_and_fit_SCR_models_with_dbinomLocal_normal.html). Fitting of SCR model using exponential detection function can be performed following similar steps (see '?dbinomLocal_EX' for further details). Below, we provide  examples to compute home range radius and area for the other detection functions available [@dey2022detfun].

### Half-normal (HN, detfun = 0)
The half-normal (HN) detection function is written as:
\begin{align}
...
The exponential (EX) detection function is written as:
\begin{align}
p_{\text{EX}}(d) = p_0 \, \exp\Big{(}-\frac{d}{\sigma}\Big{)},
\end{align}
where $p_{0} \in (0,1)$, $\sigma > 0$. Here $p_0$ denotes the baseline detection probability and $\sigma$ denotes the scale parameter. For a better understanding, we also plot the detection function against distance.
```{r , warning = FALSE, message = FALSE, fig.width = 6, fig.height = 4}
## EXPONENTIAL FUNCTION
detfun.EX <- function(D, p0, sigma){ p0*exp(-D/sigma)}
par(mfrow=c(1,1))
x <- seq(0, 15, len=10001)
plot(x, detfun.EX(D=x, p0=0.25, sigma = 2), type = "l", lwd=2, col = "orange", main = "Exponential detection function", xlab = 'd', ylab = 'Detection probability') 
lines(x, detfun.EX(D=x, p0=0.15, sigma = 3), lwd=2, lty = "solid", col = "darkcyan")
focal.vars <- c('p0', 'sigma') 
focal.values <- cbind(c(0.25, 0.15), # p0
                      c(2, 3)) # sigma
...
Next we obtain the $95\%$ home range radius and area for exponential detection function. Again, we only have to provide the parameter values for 'sigma' (i.e., 'p0' is not needed) to compute home range radius and area.
```{r , warning = FALSE, message = FALSE}
# Exponential (EX) 
sigma = 2
params <- c(sigma)
...
The asymmetric logistic (AL) detection function is written as:
\begin{align}
...
### Bimodal (BI, detfun = 4) 
The bimodal (BI) detection function is written as:
\begin{align}
...
### Donut (DN, detfun = 5) 
The donut (DN) detection function is written as:
\begin{align}
p_{\text{DN}}(d) = 
\begin{cases}
