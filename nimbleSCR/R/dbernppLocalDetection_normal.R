#' Bernoulli point process detection model with local evaluation
#' 
...
#' 
#' @param x Vector of x- and y-coordinates of a spatial point (detection location).
...
#')
#'
#'dbernppLocalDetection_normal(c(1.1, 1.2), ScaledLowerCoords$coordsDataScaled,
#'                             ScaledUpperCoords$coordsDataScaled,
#'                             s, sd, baseIntensities, windowIndex, 
...
dbernppLocalDetection_normal <- nimbleFunction(
  run = function(
    ...
    if(sum(localWindows == windowIndex) == 0){
      if(log) return(-Inf)
      else return(0.0)
    }
...
rbernppLocalDetection_normal <- nimbleFunction(
  run = function(
    ...
    print("rbernppLocalDetection_normal is not implemented.")
    returnType(double(1))
    return(s)
  }
)
