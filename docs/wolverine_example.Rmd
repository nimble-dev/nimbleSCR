...
This step consists in restricting calculations linked to detection function (here a halfnormal function) to detectors within a maximum realistic radius [@Milleret2019]. 
...
The value of $d_{max}$ should be as small as possible but always large enough so that for any particular individual, the `sxy` values of the initial activity center location restrict the calculation of $p$ to all detectors with positive detections.
...
* `y`: a second matrix of identical dimension, containing the number of detections occurring at each detector. This second matrix is necessary for modelling non-binary detections (e.g. $binomial$ observation models)
* `detNums`: a vector containing the number of detectors where each individual was detected.
...
MCMC_runtime <- system.time(
  samples <- runMCMC(Cmcmc, niter = 50)
)
```
